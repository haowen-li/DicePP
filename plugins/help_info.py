...
HELP_STR += '本骰娘的绝活包括:优劣势投骰, 强大的查询功能, 记录生命值和录入角色卡, 删除指定先攻条目等\n'
HELP_STR += '交流群:861919492'

HELP_COMMAND_UPDATE_STR = '2020/3/10 v0.5.0:\n1.新增录入角色卡和一键检定功能\n2.直接at骰娘在关闭状态下也可以响应\n3.sethp关键字被简化为hp\n4.dismiss仅在@骰娘的情况下有效\n'
HELP_COMMAND_UPDATE_STR += '2020/3/8 v0.4.1:\n1.新增点菜, 今日菜单功能\n2.加入大量可查询内容\n3.可以识别群名片作为默认昵称了\n'
HELP_COMMAND_UPDATE_STR += '2020/3/6 v0.4.0: 新增烹饪功能\n'
HELP_COMMAND_UPDATE_STR += '2020/3/4 v0.3.2:\n1.查询时可以用/区分多个关键字 2.修复了hp正好降到0时会产生的错误 3.增加了.dismiss功能 4.增加了.draw功能\n'
...
HELP_COMMAND_STR += '骰娘会自动去掉大多数空格以及转换小写, 多数指令需要后接参数, 详细用法请输入.help [指令名]查询, 如.help sethp'
...
HELP_COMMAND_QUERY_STR += '目前可查询的内容有: PHB全本(by梨子,花作噫,邪恶,赵小安,睡帽), 全拓展法术, 全拓展专长, DMG魔法物品与生物(by花作噫)\n'
HELP_COMMAND_QUERY_STR += '示例:\n'
HELP_COMMAND_QUERY_STR += '.查询 火球术\n'
HELP_COMMAND_QUERY_STR += '.查询 诗人/施法能力\n'
...
HELP_COMMAND_PC_STR =  '角色卡相关功能: .角色卡 ([可选指令]) [可选指令]:查看 (默认选项, 输出的指令可直接用来导入角色卡); 完整查看 (输出完整角色卡); '
HELP_COMMAND_PC_STR += '录入 指定格式的角色卡信息 (根据后面的内容录入角色卡); 删除 (删除当前的角色卡)\n'
HELP_COMMAND_PC_STR += '录入角色卡后, 就可以使用相关的检定与豁免功能了, 详情请输入.help检定查看\n'
HELP_COMMAND_PC_STR += f'角色卡模板:\n{pcSheetTemplate}\n'
HELP_COMMAND_PC_STR += f'提示:\n必填的内容是六个基本属性,熟练加值与熟练项,其他内容为选填\n'
HELP_COMMAND_PC_STR += '在额外加值里写"力量+1d4"的话, 会把所有相关检定,如力量检定,力量豁免,运动检定的结果都加上1d4哦~同理, 写"豁免+1",会将所有豁免的结果都加1(如幸运石)\n'
HELP_COMMAND_PC_STR += '录入角色卡时输入的名称等同于.nn [名称], 输入的生命值信息等同于.hp [生命值]/[最大生命值]\n'
HELP_COMMAND_PC_STR += '如果不确定自己的理解是否正确的话, 输入".角色卡 完整查看"确认一下吧!\n'
HELP_COMMAND_PC_STR += f'所有技能名:{list(pcSkillDict.keys())}'

HELP_COMMAND_CHECK_STR = '属性检定系列功能: .[属性检定或豁免][优/劣势][加值] [原因]\n'
